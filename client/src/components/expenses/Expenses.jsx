...
const Expenses = () => {
  ...
  if (user) {
    var { fetchedEmail, uid, name } = user.email;
  }
  const email = fetchedEmail || localStorage.getItem("email");
  const categoryHandler = (e) => {
    setCategory(e.target.value);
  };

  const getExpenses = useCallback(() => {
    fetch(`https://chat-app-891e9-default-rtdb.firebaseio.com/${email}.json`, {
      method: "GET",
      headers: {
